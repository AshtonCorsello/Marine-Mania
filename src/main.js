    ...
    if(mode == 1 | mode == 5){ // Game has started
      if(isPaused() == true){ // If the game is paused display the pause menu
        ...
        if(countDown < 0){
          ...
          if (player.level != 0 && player.score >= 100*player.level) ++player.level;
...
function GameInitialization(){ // initialization
        ...
        if(mode != 1){player.level = 1;mode = 1;} 
                          // change mode at the end to ensure all this code is processed before the code in draw:mode1 is ran.
